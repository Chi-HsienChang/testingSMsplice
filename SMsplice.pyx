...
###################################################
# 3. 單步 forward DP：僅從前一層計算下一層
###################################################
cdef dict forward_dp_step(dict F_prev, int pos, object sequences, int length,
    double pME, double[:] pELF, double[:] pIL, 
...
###################################################
# 4. forward_backward_low_memory: 利用 checkpointing 以低記憶體完成 forward-backward
###################################################
cpdef tuple forward_backward_low_memory(
    ...
    """
    此函數採用 checkpointing 技術：
      1. 前向計算時只儲存每隔 checkpoint_interval 的 F 狀態，
         其餘部分後續於 backward 區段中重算。
      2. 後向計算時，將序列依 checkpoint 分成數個區段，
         針對每個區段重算 forward DP（僅該區段內）並做 backward 遞推，
         同時計算各 pos 的 posterior。
    回傳: (post_list, logZ)
      post_list[i] = { symbol: posterior_prob }，i 為全局序列位置。
    """
    cdef dict checkpoints = {}  # 儲存 checkpoint: key = pos, value = F[pos]（dict）
    cdef dict F_current = {}
    ...
    for pos in range(0, length):
        F_current = forward_dp_step(F_current, pos, sequences, length,
    ...
    cdef double alpha_val, new_log_score, contrib
    cdef list allowed_symbols
    cdef double b_val 
    cdef double val 
    cdef double prob
    ...
    for i in range(len(ckpt_positions) - 1, 0, -1):
        ...
        # 針對此區段重新計算 forward DP（僅該區段內，記憶體佔用低）
        seg_F = [None] * (seg_len + 1)  # seg_F[0] 對應全局 pos = seg_start，seg_F[seg_len] 對應 seg_end
        seg_F[0] = checkpoints[seg_start].copy()
        for j in range(0, seg_len):
            global_pos = seg_start + j
            seg_F[j + 1] = forward_dp_step(seg_F[j], global_pos, sequences, length,
                                            pME, pELF, pIL, pEE, pELM, emissions5, emissions3)
        # 現在對該區段進行 backward 遞推：
        seg_B = [None] * (seg_len + 1)
        seg_B[seg_len] = B_next_segment  # 區段尾端的 backward 值（即 checkpoint seg_end 的 B 值）
        
        # 從區段內由後往前計算 backward（與原 backward 遞推相同）
        for j in range(seg_len - 1, -1, -1):
            ...
            if global_pos == 0 or global_pos == length - 1:
                ...
                allowed_symbols = [0, 5, 3]
            # 對 seg_F[j] 中的每個 state 計算 backward 貢獻
            for state, alpha_val in seg_F[j].items():
                contributions = []
                for symbol in allowed_symbols:
                    new_state_tuple = transition_dp(state, alpha_val, global_pos, symbol,
                                                    sequences, length,
                                                    pME, pELF, pIL, pEE, pELM,
                                                    emissions5, emissions3)
                    if new_state_tuple is None:
            ...
            seg_B[j] = B_seg
            # 利用 seg_F[j] 與 seg_B[j] 更新全局 posterior (global pos = seg_start+j)
            for state, alpha_val in seg_F[j].items():
                if state in seg_B[j]:
                    b_val = seg_B[j][state]
